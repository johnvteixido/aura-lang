...
class TestRuntime < Minitest::Test
  ...
  def run_generated(aura_source, driver)
  ...
  # The boot gate (`set :run, true`) is what actually starts the server under
  # `aura run` when the app is loaded via eval -- lock it in.
  def test_run_web_emits_boot_flag
    ...
    AURA
    assert_match(/set :run, true/, code)
  end
