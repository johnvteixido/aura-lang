...
module Aura
  ...
  class CodeGen
    ...
    def emit_train(node)
      ...
      @e.line "scheduler = Torch::Optim::LRScheduler::#{SCHEDULERS[cfg[:scheduler]]}.new(optimizer, step_size: 1)" if has_sched
    ...
    def emit_evaluate(node)
      ...
      @e.comment %(Evaluation: #{model} on "#{node[:dataset]}")
    ...
    def emit_route(node)
      ...
      @e.block("#{verb} '#{node[:path]}' do") do
        if node[:auth]
          ...
          @e.line %(halt 401 unless request.env["HTTP_AUTHORIZATION"] == "Bearer " + ENV["AURA_API_TOKEN"].to_s)
        end
    ...
    def emit_run_web(node)
      @e.comment "Boot the Sinatra server. `set :run, true` forces classic mode to"
      @e.comment "start at exit even when the app is loaded via eval/require."
